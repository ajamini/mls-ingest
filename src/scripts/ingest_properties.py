...
def ingest_properties(queue):
    item = pickle.loads(queue.dequeue().data)
    args = item[2]
    listings = args[0]
    result = []
    ...
    for property in listings:
        listing = Listing(
            source=Source(id=1),
            type=property['SaleLease'],
            # created_at=property[''],
            # last_updated=property[''],
            mls=property['MLS'],
            status=property['Status'],
            price=property['ListPrice'],
            description=property['RemarksForClients'],
            extra_description=property['Extras'],
            possession_notes=property['PossessionRemarks'],
        )
        listing.save()

        Agent(
            listing=listing,
            brokerage=property['ListBrokerage'],
            # name=property['']
        ).save()

        result.append({'MLS': listing.mls, 'identifier': listing.id})
